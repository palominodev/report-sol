...
describe('PairPolicyRule (R2 per-tipo pair policy)', () => {
  ...
  describe('mixed_iff_same_familia (empiece_conversaciones, escenificacion)', () => {
    it('allows a mixed pair when both share the same non-null familia_id', () => {
      ...
      expect(rule.isAllowed(companionCtx(candidate, presenter, 'escenificacion'))).toBe(true);
    });
    ...
    it('blocks a mixed pair when either familia_id is NULL (NULL = not family)', () => {
      ...
      expect(rule.isAllowed(companionCtx(candidate, presenter, 'escenificacion'))).toBe(false);
    });
